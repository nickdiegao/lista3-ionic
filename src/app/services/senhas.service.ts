...
// Define a interface para o objeto de senhas
interface SenhasObject {
    [key: string]: any[];
}
...
export class SenhasService {
  ...
  public senhasArray: SenhasObject = {
    'SG': [],
    'SP': [],
    'SE': []
  };
  ...
  novaSenha(tipoSenha: string = '') {
    if (tipoSenha == 'SG') {
      ...
      new Date().getMonth().toString().padStart(2, '0') +
      new Date().getDay().toString().padStart(2,'0') +
      '-' + tipoSenha + (this.senhasArray['SE'].length + 1).toString().padStart(2, '0');
